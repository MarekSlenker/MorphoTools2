...
As the `transformCharacter()` function takes as argument another function (`FUN` argument), there is an inexhaustible amount of potential transformations. Here we will mention some of them:  

For a right-skewed (positive) distribution can be used:  
* logarithmic transformation (natural): `FUN = function(x) log(x+1)`  
* logarithmic transformation (common): `FUN = function(x) log10(x+1)`  
* square root transformation: `FUN = sqrt`   
...
* logarithmic transformation (natural): `FUN = function(x) log((max(x)+1)-x)`  
* logarithmic transformation (common): `FUN = function(x) log10((max(x)+1)-x)`  
...
In the above examples, we use a number 1 as constant, but note, the chosen constant affect the result, as shown in the figure below.
...
c1 = transformCharacter(centaurea, character = "SF", FUN = function(x) log(x+0.001))
c2 = transformCharacter(centaurea, character = "SF", FUN = function(x) log(x+10))
c3 = transformCharacter(centaurea, character = "SF", FUN = function(x) log(x+1000))

stats::qqnorm(as.matrix( na.omit(c1$data["SF"])), main = "function(x) log(x+0.001)", cex = 0.9, bty="n")
stats::qqline(as.matrix( na.omit(c1$data["SF"])), lwd=2)

stats::qqnorm(as.matrix( na.omit(c2$data["SF"])), main = "function(x) log(x+10)", cex = 0.9, bty="n")
stats::qqline(as.matrix( na.omit(c2$data["SF"])), lwd=2)

stats::qqnorm(as.matrix( na.omit(c3$data["SF"])), main = "function(x) log(x+1000)", cex = 0.9, bty="n")
stats::qqline(as.matrix( na.omit(c3$data["SF"])), lwd=2)
