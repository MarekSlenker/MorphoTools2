...
Boxplots can be calculated for particular character using `boxplot.character()` or for all characters at once running `boxplotAll()` function, that saves all boxplots to a new folder in your working directory. The whiskers can be extended to desired percentiles using `lowerWhisker` and `upperWhisker` arguments. Missing data are omitted.
...
Here is the right place to mention also `deleteTaxon()` and another trio of methods with reversed logic  which will return only mentioned populations, taxa or characters: `keepPopulation()`, `keepTaxon()`, and `keepCharecter()`.
...
**Šlenker M, Koutecký P. (2020)**. MorphoTools: an R package for morphometric analysis. *Plant Systematics and Evolution*
