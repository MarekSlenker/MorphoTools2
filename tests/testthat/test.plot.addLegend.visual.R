...
test_that("addlegend visual",  {
  ...
  tmp  = tempfile(fileext = ".png")
  png(filename = tmp, width = 400, height = 400)
  plotPoints(pcoaRes, pch =c(18,16), col = c("red", "green", "blue", "black", "yellow"), cex = 2)
  plotAddLabels.points(pcoaRes, offset = 0.5, pos = 2, labels = c("7", "3", "5", "6"), include = F)
  plotAddLabels.points(pcoaRes, offset = 0.5, pos = 4, labels = c("7", "3", "5", "6"), include = T)
  plotAddLegend(pcoaRes, pch =c(24,16), x = 0, y = 0, pt.cex = 1.5,
                col = c("red", "green", "blue", "black", "navy"),
                pt.bg = "salmon", box.bg = "grey", box.lwd = 2, box.lty = 2, box.col = "#FF0000", ncol = 3)

  dev.off()
  expect_true(visualTest::isSimilar(tmp,visualTest::getFingerprint("../testFiles/figs/plotAddLegend_12.png"), threshold = 1)  )
